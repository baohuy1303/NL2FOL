...
class NL2FOL:
    ...
    """
    def __init__(self, sentence, model_type, pipeline, tokenizer, nli_model, nli_tokenizer, query_engine, debug=False, direct = False):
...
if __name__ == '__main__':
    ...
    final_lfs2 = []
    count=0
    query_engine = init_rag_query_engine()
    for i,row in df.iterrows():
        print(count)
        nl2fol=NL2FOL(row['articles'],model_type,pipeline,tokenizer,nli_model,nli_tokenizer,query_engine, debug=True)
