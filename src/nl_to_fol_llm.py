...
class NL2FOL:
    ...
    def check_entailment(self, clause1, clause2):
        with open("prompts/prompt_prop_relation.txt", encoding="ascii", errors="ignore") as f:
            prompt = f.read()
        prompt_template="""
        Clause 1: {}
        Clause 2: {}
...
if __name__ == '__main__':
    ...
    for i,row in df.iterrows():
        print(count)
        nl2fol=NL2FOL(row['articles'],model,pipeline,tokenizer,nli_model,nli_tokenizer,debug=True)
